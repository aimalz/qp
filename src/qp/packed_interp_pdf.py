...
def extract_and_pack_vals_at_x(in_dist, **kwargs):
    ...
    return dict(xvals=xvals, ypacked=ypacked, ymax=ymax, packing_type=packing_type, **kwargs)


class packed_interp_gen(Pdf_rows_gen):
    ...
    def _unpack(self):
        self._yvals = unpack_array(PackingType(self._packing_type), self._ypacked, row_max=self._ymax, log_floor=self._log_floor)
    ...
    def make_test_data(cls):
        ...
        ypacked_log, ymax_log = pack_array(PackingType.log_from_rowmax, YARRAY.copy(), log_floor=-3)

        cls.test_data = dict(
            lin_packed_interp=dict(
                gen_func=packed_interp,
                ctor_data=dict(packing_type=PackingType.linear_from_rowmax, xvals=XBINS, ypacked=ypacked_lin, ymax=ymax_lin),
                convert_data=dict(xvals=XBINS, packing_type=PackingType.linear_from_rowmax), test_xvals=TEST_XVALS
            ),
            log_packed_interp=dict(
                gen_func=packed_interp,
                ctor_data=dict(packing_type=PackingType.log_from_rowmax, xvals=XBINS, ypacked=ypacked_log, ymax=ymax_log, log_floor=-3.),
