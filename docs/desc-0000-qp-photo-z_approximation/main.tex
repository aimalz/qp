...
\COMMENT{There's way, way too much detail in the subsubsections here!}
...
object and given $N_{f}$]  \COMMENT{Should I show multiple \texttt{qp.PDF} 
objects? a different $N_{f}$? a simpler or more complex function?}
  \label{fig:qp}}
\end{figure}
...
\COMMENT{Find more examples of usage.}
...
dependent on random choices, such as random forests.  \COMMENT{Cite some 
machine learning \pz\ methods that natively produce samples.}  Such approaches 
typically produce large numbers of samples, far more than can realistically be 
stored by any survey, so a subsample is commonly stored.  \COMMENT{Cite recent 
DES paper, which stores a single sample.}
...
particularly that of a small number of Gaussian components.  \COMMENT{Seek 
citation for BPZ top 3 Gaussian components.}  A Gaussian mixture model may be a 
...
values at this time.   \COMMENT{Actually, the gridded format already does this, 
so it's not as much of a problem as I'm claiming here.}  We anticipate 
...
mixture model against other formats.  \COMMENT{This sort of weakens my 
assertion of realistic complexity with the mock data.}
...
up for a variety of science applications.  \COMMENT{I have many other ideas for 
how to use \pz s but would like to be able to cite something that's actually 
been done.}  For this purpose, we also calculate $\{KLD[p_{k}(z) | \hat{p}^{X; 
...
\COMMENT{Perhaps switch the order of the individual and stacked metrics?}
...
one panel per dataset (vertical)]  \COMMENT{A random sample of a handful of \pz 
s doesn't really tell me much about the whole dataset, but cherrypicking the 
most interesting or average members of the catalog could be more misleading.  
What else can I do to give readers a feel for the differences between the 
datasets?}
  \label{fig:pzs}}
\end{figure}
...
\COMMENT{Written by Sam Schmidt, anticipating future contributions from Joe 
DeRose, Risa Wechsler, and/or Matt Becker}
...
\COMMENT{Written by Melissa Graham, have asked for larger catalog comparable to 
Sec. \ref{sec:LSST}.}
...
of (dataset, $N_{f}$)]  \COMMENT{Which $N_{f}$ and dataset should I choose?}
...
\COMMENT{Interpreting this is pretty interesting!}
...
\includegraphics[width=0.9\columnwidth]{figures/euclid_stacked_placeholder.png}
  \caption{Placeholder!  \COMMENT{Should I show the $\hat{n}(z)$ plot or just 
the metrics?}  [$N_{f}$ (horizontal) vs. metric value (vertical), one color per 
