...
Buzzard-highres-v1.0 mock galaxy catalog (NOTE: how do we cite this?  Ask Joe 
DeRose!), which adds galaxies with SEDs drawn from an empirical library of 
\sim500,000 SEDs from the Sloan Digital Sky Survey  (SDSS) (NOTE: I need to 
look up the spec-z sample reference, we should ask Joe DeRose to make sure we 
have the right one!).  Given an SED, redshift, and absolute r-band magnitude 
for each galaxy, we compute the expected apparent magnitudes and magnitude 
errors in the six broadband LSST filters (ugrizy), assuming full 10-year depth 
of the survey using the simple model of \citet{ivezic_lsst:_2008}.  The catalog 
contains 111,171 galaxies to a depth of $i<26.9$, 1.5 magnitudes deeper than 
the expected "LSST Gold sample'' of galaxies that will have S/N$\gtrsim$30 in 
multiple bands.

To obtain redshift posteriors, we use the publicly available code 
\texttt{BPZ}\footnote{available at \url{http://www.stsci.edu/~dcoe/BPZ}} 
\citep[Bayesian Photometric-z,see][for details]{benitez_bayesian_2000}.  We 
create a custom Bayesian prior using a subset of the Buzzard-highres-v1.0 
catalog, and create a spanning template set via a simple k-means clustering 
algorithm, selection 100 of the SDSS SEDs used in creating the Buzzard catalog. 
 BPZ outputs the marginalized redshift posterior on a regular grid of test 
redshifts, in this case we have 211 points spanning $0.005\leqz\leq2.105$.
...
of multi-modal \pz's; some examples are shown in the left panel of 
Fig.~\ref{fig:pzs}.  We produce "true'' underlying PDFs by fitting a 
five-component Gaussian mixture model to each \pz output by BPZ.


\subsection{Euclid mocks}
\label{sec:mg}

We start with a 30000 object subset of the same simulated galaxy catalog used 
...
$i$-band magnitude of $i<25$ and true redshifts $z<3.5$. As in Graham et al. we 
simulate observed apparent magnitudes from the true catalog magnitudes by 
adding a normal random scatter with a standard deviation equal to the predicted 
magnitude error for each galaxy (from Section 3.2.1. of 
...
accumulation of 56, 80, 184, 184, 160, and 160 visits in filters $ugrizy$). We 
also ignore any magnitudes fainter than the predicted 10-year limiting 
magnitudes in each filter, $u<26.1$, $g<27.4$, $r<27.5$, $z<26.1$, and 
$y<24.9$, as a realistic simulation of non-detections. We estimate photometric 
redshifts for this simulated catalog using the Bayesian Photometric Redshift 
Estimator (BPZ; \citealt{benitez_bayesian_2000}), using the CFHTLS set of 
spectra \citep{ilbert_accurate_2006} and all of the default parameter settings 
for BPZ, except that we impose a maximum photometric redshift of 3.5 and allow 
BPZ to use the $i$-band as a magnitude prior during the photo-$z$ fit. The 
photo-z PDFs from BPZ are in the form of $N_{ff} = 351$ floating point numbers 
represing the probability on a regular grid of redshifts $0.01 < z_{\rm phot} < 
3.51$.
