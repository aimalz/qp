...
interpolation scheme $F$ operating on another function $\Phi(z)=p$, such as a 
kernel density estimator (KDE), that transforms samples into probability 
density estimates, which also has metaparameters $\vec{C}''$ such as a 
smoothing bandwidth in the case of a KDE.  The samples representation is then
\begin{align}
  \label{eq:sampled}
...
values at this time.   We anticipate including the sparse basis representation 
in \qp\ in future work when we can justify a choice of how to handle negative 
values.
...
\COMMENT{Mention interpolation choices for evaluating metrics.}
