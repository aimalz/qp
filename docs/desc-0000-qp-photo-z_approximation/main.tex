...
\textbf{Given that the \pz\ catalogs of ongoing surveys already include 
$\sim10^{7}$ galaxies, and that those of upcoming surveys will include 
$\sim10^{10}$ galaxies, storage of these probability distributions must balance 
...
\textbf{Samples are easy to use in standard science applications developed for 
redshift point estimates, so they have an established presence in the 
...
regular grid of $N_{ff}=211$ redshifts $0.005\leq z\leq2.105$, \textbf{Our 
analysis characterizes the results for ten instantiated catalogs using the 
interquartile range (IQR), with lower and upper bounds representing the 
$25^{\mathrm{th}}$ and $75^{\mathrm{th}}$ percentiles, as a model-independent 
error characterization.}a subsample of which are plotted in the right panel of 
...
\begin{figure*}
  \begin{center}
    ...
    \caption{
    \textbf{The medians of the mean ($\pentagon$), variance ($\bigstar$), and 
...
line) formats with IQR error bars based on 10 instantiations of 100 galaxies, 
...
as $N_{f}$ increases, \textbf{indicating convergence toward Gaussianity with 
increasing $N_{f}$}.
