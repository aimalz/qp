...
respectively).
We also calculate the percent error on the moments of each \pz\ under all 
parametrizations and use the median and interquartile range of the moment 
percent error distribution as another metric of the accuracy of individual \pz\ 
parametrizations.
...
error on the first three moments of $\hat{n}(z)$.
...
components was chosen to equal the median modality of the gridded \pz s.
...
KLD calculated for every \pz\ in the dataset's ensemble.
...
border) formats.
    For the individual \pz s, the piecewise constant format has a lower median 
...
readability.
    Left panel: The histogram format consistently minimizes the moments of the 
log-KLD distribution of the \mgdata data mock catalog at low $N_{f}$, and the 
moments are similar across formats at the highest $N_{f}=100$ with little 
evolution with number of stored parameters.
    Right panel: The\ssdata data mock catalog also shows the histogram format 
performing well at low numbers of stored parameters with little evolution with 
increasing $N_{f}$.
    \label{fig:moments}}
  \end{center}
\end{figure*}
