...
Modern galaxy surveys produce redshift probability density functions (PDFs) in 
addition to traditional photometric redshift (photo-$z$) point estimates.
...
we encourage further consideration of these formats for PDF approximation.
...
density function (PDF) conveying the potential for such systematic errors for 
each galaxy in the survey \citep{tanaka_photometric_2018, de_jong_third_2017, 
...
galaxies, storage of these PDFs \textbf{must balance accuracy of the catalog} 
against limited storage resources.
...
characterizing the redshift \citep[Section 4.2.2]{juric_lsst_2017}, including 
...
package\footnote{\url{https://github.com/aimalz/qp}} \citep{malz_qp_2017} 
enabling each survey to optimize their \pz\ approximation via mathematically 
...
\texttt{numpy}\footnote{\url{http://www.numpy.org/}} \citep{walt_numpy_2011}, 
\texttt{scipy} \footnote{\url{https://www.scipy.org/}} scipy 
\citep{jones_scipy:_2001}, and 
...
A subset of the included metrics is described in Section~\ref{sec:metric}.
...
However, no galaxy survey mission has yet proposed this scheme, so we could 
thus only speculate as to the potential scope and specific goals of such an 
...
which may be general mixture models of PDFs from a comprehensive library of 
those with functional forms implemented as \texttt{scipy.rv\_continuous} 
objects.}
...
enforce the condition that the representation be a PDF in the mathematical 
sense of nonnegativity and integration to unity.
...
\citep{sheldon_photometric_2012, tanaka_photometric_2018, de_jong_third_2017}.
...
with a very broad or compact PDF may have many parameters taking the same value 
$c_{i} \approx (C_{N_{f}+1} - C_{1}) / \delta$ or $c_{i} \approx 0$, for broad 
and compact PDFs respectively, that are redundant in storage.
...
C_{N_{f}})} z\right)$ & $z > c_{N_{f}+1}$
  ...
  \right\}.
\end{align}
In this study, we also restrict consideration to regular quantiles $C_{i} 
\equiv i / (N_{f} + 1)$, though \qp\ supports arbitrary quantile spacing.
...
approximation of the PDF $\hat{P}(z)$ instead of the reference PDF $P(z)$, 
given by the Kullback-Leibler divergence (KLD), which is defined as
\begin{align}
  \label{eq:kld}
...
approximate $P(z)$.
The KLD requires that both functions $P(z)$ and $\hat{P}(z)$ be PDFs (always 
positive semidefinite and integrating to unity); this may need to be explicitly 
enforced for some approximation formats.
...
In the Appendix, we review the properties of the KLD and establish some 
intuition for it.
...
\end{align}
of a PDF.
We note that $M_{0}[P]=1$ for all properly normalized PDFs, $M_{1}[P]=\bar{z}$ 
is the \textit{mean}, $M_{2}[P]$ is the \textit{variance}, and $M_{3}[P]$ is 
the \textit{skewness}.
Though the first few moments are not in general sufficient to characterize a 
highly structured PDF, they are included in this analysis because they can 
prove useful in setting ballpark estimates of the influence of different 
systematics in various science cases.
...
\textit{from} \textbf{the approximation} (the stacked estimator of a catalog of 
evaluations of reconstructed \pz s) \textit{to} \textbf{the original} (the 
stacked estimator of a catalog of evaluations of the high-resolution reference 
...
\textbf{All \pz s were fit to simulated LSST 10-year $ugrizy$ apparent 
magnitudes and errors \citep{ivezic_lsst:_2008}} using the publicly available 
Bayesian Photometric Redshift (BPZ) code \citep{benitez_bayesian_2000}, which 
employs fitting to a library of spectral energy distribution (SED) templates.
...
accuracy of \pz s produced by different methods.]{tanaka_photometric_2018, 
...
posterior probability density that is a continuous function, to which the 
...
\citet{graham_photometric_2018} simulated galaxy catalog used for LSST 
...
observed apparent $ugrizy$ magnitudes from the true apparent magnitudes and the 
10-year LSST observational errors of \citet{ivezic_lsst:_2008} before imposing 
cuts in true redshifts $z<3.5$ and observed apparent magnitudes fainter than 
the predicted 10-year limiting magnitudes in each filter ($u<26.1$, $g<27.4$, 
$r<27.5$, $i<25$, $z<26.1$, and $y<24.9$) to realistically simulate 
non-detections.}
...
\textbf{In a science application that is robust to \pz\ outliers, a 
parametrization with a high variance or skewness may be acceptable, whereas 
another science application that simply requires a well-characterized error 
distribution may tolerate a higher mean in exchange for lower variance and 
skewness.}
...
galaxies, which are offset about $N_{f}$ to improve readability.
    Left panel: The moments of the distribution of individual \pz\ log-KLD 
values of the \mgdata mock catalog are minimized when they are stored as 
samples, except at large $N_{f}$.
...
showing the histogram format minimizes the moments at all but the smallest 
$N_{f}$.
...
tails; our choise of PDF reconstruction method for the quantile format is most 
susceptible to error in the tails of the PDF, and only the histogram format 
preserves information uniformly at all redshifts rather than at all 
probabilities.
...
\begin{figure*}
  \begin{center}
    ...
    \caption{
    % referee report response 1
    The \textbf{median} log$_{10}$-percent errors on the mean ($\pentagon$), 
...
instantiations of 100 galaxies, where the log$_{10}$-percent errors and their 
...
Though the log$_{10}$-percent error of the moments of individual \pz s also 
...
\textbf{The unwaveringly linear marginal improvement in the log-percent error 
of the per-galaxy \pz\ moments with increasing log-$N_{f}$ may reflect the fact 
that samples are taken randomly in the space of probability and thus unaffected 
by the interpolation artifacts of the quantile format and the interplay between 
the scales of features and bins of the histogram format.}
...
of compressing \pz\ catalogs and emphasize that any decision should \textbf{be 
motivated by science requirements and} account for the absolute, relative, and 
marginal behavior of the formats considered as a function of the number of 
stored parameters.
...
Mellon University.
AIM is advised by David W. Hogg and was supported by National Science 
Foundation grant AST-1517237.
...
Cavuoti, Maciej Bilicki, Johann Cohen-Tanugi, Eric Gawiser, Daniel Gruen, and 
Seth Digel for helpful feedback in the preparation of this paper.
AIM thanks Coryn Bailer-Jones, Eric Ford, and Morgan Fouesneau for providing 
context for this work outside cosmology.
...
numpy \citep{walt_numpy_2011},
pathos \citep{mckerns_building_2012, mckerns_pathos:_2010},
qp \citep{malz_qp_2017},
scikit-learn \citep{pedregosa_scikit-learn:_2011},
scipy \citep{jones_scipy:_2001}
}
