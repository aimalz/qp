"""This small module implements generic tests for distributions to ensure that they have been implemented consistently"""
...
def assert_all_close(arr, arr2, **kwds):
    ...
    if not np.allclose(arr, arr2, **kwds): #pragma: no cover
        raise ValueError("%s %.2e %.2e %s" % (test_name, (arr-arr2).min(), (arr-arr2).max(), kwds))
...
def run_pdf_func_tests(test_class, test_data, short=False, check_props=True):
    ...
    if ctor_func is None: #pragma: no cover
        raise KeyError("failed to find creation method %s for class %s" % (method, test_class))
    try:
